   ...
   <table>
      ...
      <tbody>
         @foreach ($especialidades->sortBy('nombre')->values() as $i => $especialidad)
            <tr>
               ...
               @foreach ($fiscalias->sortBy('nombre') as $j => $fiscalia)
                  <td> {{$atendidas->where('fiscalia2_id',$fiscalia->id)->where('unidad_id',1)->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->whereIn('documento_emitido',['dictamen','certificado'])->count()}} </td>
               @endforeach
               <td> {{$atendidas->where('unidad_id',1)->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->whereIn('documento_emitido',['dictamen','certificado'])->count()}} </td>
            </tr>
         @endforeach
         <tr>
            <td>{{$i+2}}</td>
            ...
            @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
               <td> {{$atendidas->where('fiscalia2_id',$fiscalia->id)->where('unidad_id',1)->where('documento_emitido','informe')->count()}} </td>
            @endforeach
            <td> {{$atendidas->where('unidad_id',1)->where('documento_emitido','informe')->count()}} </td>
         </tr>
		 <tr>
            <td>{{$i+3}}</td>
            <td style="background-color: orange;">Colaboraciones</td>
            @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
               <td> {{$atendidas->where('fiscalia2_id',$fiscalia->id)->where('unidad_id',1)->where('documento_emitido','colaboraciones')->count()}} </td>
            @endforeach
            <td> {{$atendidas->where('unidad_id',1)->where('documento_emitido','colaboraciones')->count()}} </td>
         </tr>
         {{-- <tr>
            <td>{{$i+3}}</td>
            <td style="background-color: orange;">Archivo</td>
            @foreach ($fiscalias->sortBy('nombre') as $fiscallia)
               <td> {{$atendidas->where('fiscalia2_id',$fiscalia->id)->where('unidad_id',1)->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->where('documento_emitido','archivo')->count()}} </td>
            @endforeach
         ...
         <tr>
            ...
            @foreach ($fiscalias->sortBy('nombre') as $fiscallia)
               <td> {{$atendidas->where('fiscalia2_id',$fiscalia->id)->where('unidad_id',1)->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->where('documento_emitido','salida_juzgado')->count()}} </td>
            @endforeach
