         ...
         <tbody>
            ...
            @foreach ($especialidad->solicitudes->sortBy('nombre') as $solicitud)
               <tr>
                  ...
                  @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
                     <td>{{$atendidas->where('fiscalia2_id',$fiscalia->id)->where('unidad_id',1)->where('solicitud_id',$solicitud->id)->whereIn('documento_emitido',['dictamen','certificado'])->count()}}</td>
                  @endforeach
            ...
            <tr>
               ...
               <td>INFORMES</td>
                  @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
                        <td>{{$atendidas->where('fiscalia2_id',$fiscalia->id)->where('unidad_id',1)->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->where('documento_emitido','informe')->count()}}</td>
                  @endforeach
               <td>{{$atendidas->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->where('documento_emitido','informe')->count()}}</td>
            </tr>
			
			@if($especialidad->id == 24)
			<tr>
               <td>{{$n++}}</td>
               <td>COLABORACIONES</td>
                  @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
                        <td>{{$atendidas->where('fiscalia2_id',$fiscalia->id)->where('unidad_id',1)->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->where('documento_emitido','colaboraciones')->count()}}</td>
                  @endforeach
            ...
            <tr>
               ...
               <td>TOTAL</td>
               @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
                  <td>{{$atendidas->where('fiscalia2_id',$fiscalia->id)->where('unidad_id',1)->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->whereIn('documento_emitido',['dictamen','certificado','informe','colaboraciones'])->count()}}</td>
               @endforeach
               <td>{{ number_format($atendidas->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->whereIn('documento_emitido',['dictamen','certificado','informe','colaboraciones'])->count() ) }}</td>
            </tr>
         </tbody>   
      </table>
   @endforeach
