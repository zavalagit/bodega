      ...
      <tbody>
         ...
         @foreach ($especialidad->solicitudes->sortBy('nombre') as $solicitud)
            <tr>
               ...
               @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
                  <td>{{$atendidas->where('fiscalia2_id',$fiscalia->id)->where('unidad_id',2)->where('solicitud_id',$solicitud->id)->whereIn('documento_emitido',['dictamen','certificado'])->count()}}</td>
               @endforeach
               <td>{{$atendidas->where('solicitud_id',$solicitud->id)->whereIn('documento_emitido',['dictamen','certificado'])->count()}}</td>
            </tr>
         @endforeach
         ...
         <tr>
            ...
            <td>INFORMES</td>
               @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
                   <td>{{$atendidas->where('fiscalia2_id',$fiscalia->id)->where('unidad_id',2)->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->where('documento_emitido','informe')->count()}}</td>
               @endforeach
		...
		<tr>
            ...
            <td>JUZGADOS</td>
               @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
                   <td>{{$atendidas->where('fiscalia2_id',$fiscalia->id)->where('unidad_id',2)->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->where('documento_emitido','salida_juzgado')->count()}}</td>
               @endforeach
		 ...
		 <tr>
            ...
            <td>ARCHIVO</td>
               @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
                   <td>{{$atendidas->where('fiscalia2_id',$fiscalia->id)->where('unidad_id',2)->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->where('documento_emitido','archivo')->count()}}</td>
               @endforeach
         ...
         <tr>
            ...
            <td>TOTAL</td>
            @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
               <td>{{$atendidas->where('fiscalia2_id',$fiscalia->id)->where('unidad_id',2)->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->whereIn('documento_emitido',['dictamen','certificado','informe','salida_juzgado','archivo'])->count()}}</td>
            @endforeach
            <td>{{ number_format($atendidas->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->whereIn('documento_emitido',['dictamen','certificado','informe','salida_juzgado','archivo'])->count() ) }}</td>
         </tr>
      </tbody>   
   </table>
