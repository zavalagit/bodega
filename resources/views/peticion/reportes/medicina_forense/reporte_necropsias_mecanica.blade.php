         ...
         @php $n = 1; @endphp
            ...
            <tr>
              ...
              <td>{{$necros->where('fiscalia2_id',4)->where('unidad_id',2)->whereIn('necropsia_id',$necropsias->where('necropsia_clasificacion_id',$necropsia_clasificacion->id)->pluck('id'))->count()}}</td>
               ...
               @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
                  @continue($fiscalia->id == 4)
                  <td>{{$necros->where('fiscalia2_id',$fiscalia->id)->whereIn('necropsia_id',$necropsias->where('necropsia_clasificacion_id',$necropsia_clasificacion->id)->pluck('id'))->count()}}</td>
               @endforeach
                  <td>{{$necros->whereIn('necropsia_id',$necropsias->where('necropsia_clasificacion_id',$necropsia_clasificacion->id)->pluck('id'))->count()}}</td>
