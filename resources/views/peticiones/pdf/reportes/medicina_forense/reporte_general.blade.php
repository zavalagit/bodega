      ...
      <tbody>
         ...
         @foreach ($especialidades->sortBy('nombre') as $especialidad)
               <tr>
                  ...
                  @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
                     <td>{{ number_format( $atendidas->where('fiscalia2_id',$fiscalia->id)->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->whereIn('documento_emitido',['dictamen','certificado'])->count() )}}</td>
                  @endforeach
          ...
          @endforeach
            ...
            <tr>
               ...
               <td>Informes</td>
               @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
                  <td>{{ number_format( $atendidas->where('fiscalia2_id',$fiscalia->id)->where('documento_emitido','informe')->count() )}}</td>                    
               @endforeach
               <!--TOTAL-->
            ...
            <tr>
               ...
               @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
                  <td>{{ number_format( $atendidas->where('fiscalia2_id',$fiscalia->id)->where('documento_emitido','salida_juzgado')->count() )}}</td>                    
               @endforeach
               <!--TOTAL-->
            ...
            <tr>
               ...
               <td>TOTAL</td>
               @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
                  <td>{{ number_format( $atendidas->where('fiscalia2_id',$fiscalia->id)->whereIn('documento_emitido',['dictamen','certificado','informe','salida_juzgado'])->count())}}</td>
               @endforeach
               <td>{{number_format( $atendidas->whereIn('documento_emitido',['dictamen','certificado','informe','salida_juzgado'])->count() )}}</td>
            </tr>
