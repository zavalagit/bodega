         ...
         <tbody>
            @foreach ($especialidades->sortBy('nombre')->values() as $i => $especialidad)
               <tr>
                  ...
                  @foreach ($fiscalias->sortBy('nombre') as $j => $fiscalia)
                     <td> {{$atendidas->where('fiscalia2_id',$fiscalia->id)->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->where('documento_emitido','dictamen')->count()}} </td>
                  @endforeach
            ...
            <tr>
               ...
               @foreach ($fiscalias->sortBy('nombre') as $fiscallia)
                  <td> {{$atendidas->where('fiscalia2_id',$fiscalia->id)->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->whereIn('documento_emitido',['informe','certicado'])->count()}} </td>
               @endforeach
               <td> {{$atendidas->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->whereIn('documento_emitido',['informe','certicado'])->count()}} </td>
            </tr>
            {{-- <tr>
               <td>{{$i+3}}</td>
