<div class="row">
   <div class="col s11">
      <div class="input-field col s12">
         <input type="hidden" id="{{$input_hidden}}" name="baja_entrega" value="{{ ($accion == 'editar') ? $baja->user_id : Auth::user()->id}}">
         <input type="text" class="autocomplete" id="{{$input_text}}"
            readonly
            data-input-hidden="{{$input_hidden}}"
            data-tabla="users"
            data-user-tipo="{{$user_tipo}}"
            value="{{ ($accion == 'editar') ? "{$baja->user->folio} - {$baja->user->name}" : Auth::user()->folio." - ".Auth::user()->name}}"
         >
